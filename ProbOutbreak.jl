...
Simulation = :SP_NoAge_Dispersion_1_5_NoNPI_Fractal_Discrete #:Manaus_1_5_Dispersion_NoNPI_Discrete #:SPHomog_InLoco_Discrete
...
mkdir("Results/OutbreakProb_$(Simulation)_Nsim_$(Nsim)")
figure(1);clf()
stem(k, ProbOutbreak)
xlabel(L"Dispersion factor $k$")
ylabel(L"$P(\{$Outbreak$\})$")
title("Outbreak probability vs. dispersion factor")
savefig("Results/OutbreakProb_$(Simulation)_Nsim_$(Nsim)/OutbreakProb_$(Simulation)_Nsim_$(Nsim).svg")
figure(2);clf()
stem(k, AttackRate)
xlabel(L"Dispersion factor $k$")
...
if SAVEPLOT
    savefig("Results/OutbreakProb_$(Simulation)_Nsim_$(Nsim)/AttackRate_$(Simulation)_Nsim_$(Nsim).svg")
end
JLD.save("Results/OutbreakProb_$(Simulation)_Nsim_$(Nsim)/OutbreakProb_$(Simulation)_Nsim_$(Nsim).jld", "ProbOutbreak", ProbOutbreak, "AttackRate", AttackRate, "Simulation", Simulation, "N0", N0, "Nsim", Nsim, "caseopt", caseopt, "k", k)
